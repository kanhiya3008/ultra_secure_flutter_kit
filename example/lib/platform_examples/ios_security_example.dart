...
/// iOS-specific security example demonstrating all iOS security features
class IOSSecurityExample extends StatefulWidget {
  const IOSSecurityExample({super.key});

  @override
  State<IOSSecurityExample> createState() => _IOSSecurityExampleState();
}

class _IOSSecurityExampleState extends State<IOSSecurityExample> {
  final UltraSecureFlutterKit _securityKit = UltraSecureFlutterKit();
  bool _isInitialized = false;
  DeviceSecurityStatus? _deviceStatus;
  String _statusMessage = 'Initializing iOS security...';
  ...
  void initState() {
    super.initState();
    _initializeIOSSecurity();
    _setupSecurityListeners();
  }

  /// Initialize iOS-specific security protection
  Future<void> _initializeIOSSecurity() async {
    try {
      ...
      final config = SecurityConfig(
        ...
        allowedCertificates: [
          // Add your iOS app's certificate fingerprints
        ...
        customRules: {
          ...
          'checkForSileo': true,
          'checkForZebra': true,
          'checkForAltStore': true,
          'checkForEmulator': true,
          'checkForDebugger': true,
          'checkForSimulator': true,
          'checkForTestFlight': false, // Allow TestFlight for development
        },
      ...
      // Initialize iOS security
      await _securityKit.initializeSecureMonitor(config);

      // Enable iOS-specific security features
      await _enableIOSSecurityFeatures();

      setState(() {
        _isInitialized = true;
        _statusMessage = 'iOS security initialized successfully!';
      });
    } catch (e) {
      setState(() {
        _statusMessage = 'Failed to initialize iOS security: $e';
      });
    }
  }

  /// Enable iOS-specific security features
  Future<void> _enableIOSSecurityFeatures() async {
    try {
      ...
      print('Failed to enable iOS security features: $e');
    }
  }
  ...
  /// Test iOS-specific security features
  Future<void> _testIOSSecurity() async {
    setState(() {
      _isTesting = true;
      _testResults.clear();
      _testLogs.clear();
      _statusMessage = 'Running comprehensive iOS security tests...';
    });

    try {
      _addTestLog('🚀 Starting comprehensive iOS security tests...');
      ...
      _addTestLog('🛡️ Testing jailbreak detection...');
      final jailbreakResult = await _testJailbreakDetection();
      _testResults['jailbreakDetection'] = jailbreakResult;
      _addTestLog(
        '✅ Jailbreak detection test: ${jailbreakResult['success'] ? "PASSED" : "FAILED"}',
      );
      ...
      setState(() {
        _isTesting = false;
        _statusMessage =
            'All iOS security tests completed! $passedTests/$totalTests tests passed.';
      });

      _addTestLog(
        '🎉 Comprehensive iOS security testing completed successfully!',
      );
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _testSSLPinning() async {
    try {
      ...
      final response = await _securityKit.secureApiCall(
        'https://api.example.com/ios-test',
        {'platform': 'ios', 'test': 'ssl_pinning'},
  ...
  /// Test jailbreak detection
  Future<Map<String, dynamic>> _testJailbreakDetection() async {
    try {
      print(
        'Jailbreak detection test: Check device security status for details',
      );
      return {'success': true, 'message': 'Jailbreak detection test passed'};
    } catch (e) {
      print('Jailbreak detection test failed: $e');
      return {
        'success': false,
        'message': 'Jailbreak detection test failed: $e',
      };
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('iOS Security Example'),
          ...
          children: [
            Row(
              children: [
                Icon(
                  ...
                  color: _isInitialized ? Colors.blue : Colors.orange,
                ),
            ...
            LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation<Color>(
                _isInitialized ? Colors.blue : Colors.orange,
              ),
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildFeatureRow('Jailbreak Detection', true),
            _buildFeatureRow('Face ID / Touch ID', true),
      ...
      child: Row(
        children: [
          Icon(
            ...
            color: isEnabled ? Colors.blue : Colors.red,
            size: 20,
          ),
          const SizedBox(width: 8),
      ...
      child: Row(
        children: [
          Icon(
            ...
            color: status ? Colors.red : Colors.blue,
            size: 16,
          ),
          const SizedBox(width: 8),
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(
                  _isTesting ? 'Testing in Progress...' : 'iOS Test Results',
                  style: Theme.of(context).textTheme.titleLarge,
                ),
              ],
            ),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _isTesting ? 'Testing...' : 'Test All iOS Security',
                ),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.blue,
                  foregroundColor: Colors.white,
            ...
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                onPressed: _isInitialized ? _testJailbreakDetection : null,
                icon: const Icon(Icons.shield),
                label: const Text('Test Jailbreak Detection'),
                style: ElevatedButton.styleFrom(
